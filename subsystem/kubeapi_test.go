...
var (
	imageSetsData = map[string]string{
		"openshift-v4.14.0-arm":   "quay.io/openshift-release-dev/ocp-release:4.14.6-aarch64",
		"openshift-v4.14.0-multi": "quay.io/openshift-release-dev/ocp-release:4.14.0-multi",
...
var _ = Describe("[kube-api]cluster installation", func() {
	...
	It("deploy nutanix platform", func() {
		...
		aciSpec.ImageSetRef = imageSetRef4_14
		deployClusterImageSetCRD(ctx, kubeClient, aciSpec.ImageSetRef)
		deployClusterDeploymentCRD(ctx, kubeClient, clusterDeploymentSpec)
		deployInfraEnvCRD(ctx, kubeClient, infraNsName.Name, infraEnvSpec)
	...
	It("[kube-cpu-arch]mismatch cpu architecture between infra-env and bound cluster", func() {
		By("deploy cluster with openshiftVersion 4.14 and x86_64")
		imageSetRef4_14 := &hivev1.ClusterImageSetReference{
			Name: "openshift-v4.14.0",
		}
		aciSpec.ImageSetRef = imageSetRef4_14
		...
		By("deploy infraenv with a reference to openshiftVersion 4.14 cluster and arm64")
		infraEnvSpec.CpuArchitecture = "arm64"
		deployInfraEnvCRD(ctx, kubeClient, infraNsName.Name, infraEnvSpec)
	...
	It("[multiarch] Create multiarch cluster and bind infraenvs", func() {
		By("deploy cluster with openshiftVersion 4.14 and multiarch")
		imageSetRef4_14 := &hivev1.ClusterImageSetReference{
			Name: "openshift-v4.14.0-multi",
		}
		aciSpec.ImageSetRef = imageSetRef4_14
		deployClusterImageSetCRD(ctx, kubeClient, aciSpec.ImageSetRef)
		deployClusterDeploymentCRD(ctx, kubeClient, clusterDeploymentSpec)
		...
		Expect(cluster.OcpReleaseImage).Should(Equal("quay.io/openshift-release-dev/ocp-release:4.14.0-multi"))
	...
	It("deploy clusterDeployment with NoProxy wildcard - OpenshiftVersion does support NoProxy wildcard", func() {
		...
		Expect(cluster.OcpReleaseImage).Should(Equal("quay.io/openshift-release-dev/ocp-release:4.14.0-x86_64"))
	})
